    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (users != null)
            {
                ...
                {
                    string sql = "SET IDENTITY_INSERT SchemaOne.Users ON;"
                                    + "INSERT INTO SchemaOne.Users (UserId"
                                    + ",FirstName"
                                    + ",LastName"
                                    + ",Email"
                                    + ",Gender"
                    ...
                    foreach (Users singleUser in users)
                    {
                        ...
                        if ((sql + sqlToAdd).Length > 4000)
                        {
                            ...
                            sql = "SET IDENTITY_INSERT SchemaOne.Users ON;"
                                    + "INSERT INTO SchemaOne.Users (UserId"
                                    + ",FirstName "
                                    + ",LastName"
                                    + ",Email"
            ...
            dataContextDapper.ExecuteSQL("SET IDENTITY_INSERT SchemaOne.Users OFF");
            ...
            dataContextDapper.ExecuteSQL("TRUNCATE TABLE SchemaOne.UserSalary");

            if (userSalary != null)
            {
                using (IDbConnection dbConnection = new SqlConnection(config.GetConnectionString("DefaultConnection")))
                {
                    string sql = "INSERT INTO SchemaOne.UserSalary (UserId"
                                    + ",Salary)"
                                    + "VALUES";
                    foreach (UserSalary singleUserSalary in userSalary)
                    {
                        ...
                        {
                            dataContextDapper.ExecuteProcedureMulti(sql.Trim(','), dbConnection);
                            sql = "INSERT INTO SchemaOne.UserSalary (UserId"
                                    + ",Salary)"
                                    + "VALUES";
                        }
                        sql += sqlToAdd;
            ...
            dataContextDapper.ExecuteSQL("TRUNCATE TABLE SchemaOne.UserJobInfo");

            if (userJobInfo != null)
            {
                using (IDbConnection dbConnection = new SqlConnection(config.GetConnectionString("DefaultConnection")))
                {
                    string sql = "INSERT INTO SchemaOne.UserJobInfo (UserId"
                                    + ",Department"
                                    + ",JobTitle)"
                                    + "VALUES";
                    foreach (UserJobInfo singleUserJobInfo in userJobInfo)
                    {
                        ...
                        {
                            dataContextDapper.ExecuteProcedureMulti(sql.Trim(','), dbConnection);
                            sql = "INSERT INTO SchemaOne.UserJobInfo (UserId"
                                    + ",Department"
                                    + ",JobTitle)"
                                    + "VALUES";
                        }
